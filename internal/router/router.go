...
func New(orderHandler *handlers.OrderHandler, adminHandler *handlers.AdminHandler) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		// Aquí podrías añadir la ruta para GetOrderById si la necesitas en el frontend
		// api.GET("/orders/:id", orderHandler.GetOrderByIdHandler)

		// --- AÑADIR RUTAS DE ADMINISTRACIÓN ---
		// NOTA: En un sistema real, este grupo debería estar protegido por un middleware
		// que verifique si el usuario es un administrador.
